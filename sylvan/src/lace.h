...
/**
 * Start Lace with <max_workers> workers and a a task deque size of <dqsize> per worker.
 * If <dqsize> is est to 0, uses a reasonable default value.
 */
void lace_start(unsigned int max_workers, size_t dqsize, uint64_t helper_n);
...
extern unsigned int lace_n_workers_alive;
extern unsigned int lace_n_workers_id;
