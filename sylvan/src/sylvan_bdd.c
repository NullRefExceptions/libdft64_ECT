...
TASK_IMPL_3(BDD, sylvan_and, BDD, a, BDD, b, BDDVAR, prev_level)
{
    ...
    uint64_t count_low = MTBDD_GETNODE(low)->a& 0x1FFFFF0000000000;
    uint64_t count_high = MTBDD_GETNODE(high)->a& 0x1FFFFF0000000000;
    //uint64_t count = count_low > count_high ? count_low:count_high;
    uint64_t count = count_low + count_high;
    count += 0x10000000000;
    result = mtbdd_makenode(level, low, high|count);
#else
...
TASK_IMPL_3(BDD, sylvan_and_nogc, BDD, a, BDD, b, BDDVAR, prev_level)
{
    /* Terminal cases */
    if (a == sylvan_true) return b;
    if (b == sylvan_true) return a;
    ...
    if (aHigh == sylvan_true) {
        ...
        SPAWN(sylvan_and_nogc, aHigh, bHigh, level);
        n=1;
    }
    ...
    if (aLow == sylvan_true) {
        ...
        low = CALL(sylvan_and_nogc, aLow, bLow, level);
    }

    if (n) {
        high = SYNC(sylvan_and_nogc);
    }
    ...
    uint64_t count = count_low + count_high;
    count += 0x10000000000;
    result = mtbdd_makenode_nogc(level, low, high|count);
#else
    result = mtbdd_makenode_nogc(level, low, high);
#endif
...
TASK_IMPL_3(BDD, sylvan_and_nogc_alone, BDD, a, BDD, b, BDDVAR, prev_level)
{
    /* Terminal cases */
    if (a == sylvan_true) return b;
    if (b == sylvan_true) return a;
    ...
    if (aHigh == sylvan_true) {
        ...
        high = CALL(sylvan_and_nogc_alone, aHigh, bHigh, level);
    }
    ...
    if (aLow == sylvan_true) {
        ...
        low = CALL(sylvan_and_nogc_alone, aLow, bLow, level);
    }
...
TASK_IMPL_3(BDD, sylvan_and_alone, BDD, a, BDD, b, BDDVAR, prev_level)
{
    /* Terminal cases */
    if (a == sylvan_true) return b;
    if (b == sylvan_true) return a;
    ...
    if (aHigh == sylvan_true) {
        ...
        high = CALL(sylvan_and_alone, aHigh, bHigh, level);
        n=1;
    }
    ...
    if (aLow == sylvan_true) {
        ...
        if (n) {bdd_refs_push(high);}     
        low = CALL(sylvan_and_alone, aLow, bLow, level);
        if (n) {bdd_refs_pop(1);} 
    }
...
TASK_IMPL_4(BDD, sylvan_and_alone_profile, BDD, a, BDD, b, BDDVAR, prev_level,uint64_t *,count_addr)
{
    (*count_addr)++;
    /* Terminal cases */
    if (a == sylvan_true) return b;
    ...
    if (aHigh == sylvan_true) {
        ...
        high = CALL(sylvan_and_alone_profile, aHigh, bHigh, level,count_addr);
        n=1;
    }
    ...
    if (aLow == sylvan_true) {
        ...
        if (n) {bdd_refs_push(high);}     
        low = CALL(sylvan_and_alone_profile, aLow, bLow, level,count_addr);
        if (n) {bdd_refs_pop(1);} 
    }
