...
MTBDD _mtbdd_makenode_nogc(uint32_t var, MTBDD low, MTBDD high);
static inline MTBDD mtbdd_makenode_nogc(uint32_t var, MTBDD low, MTBDD high)
{
#if MTBDD_NODE_COUNTING
    return low == (high & 0xE00000FFFFFFFFFF)? low : _mtbdd_makenode_nogc(var, low, high);
#else
    return low == high ? low : _mtbdd_makenode_nogc(var, low, high);
#endif
}
/**
...
{
#if MTBDD_NODE_COUNTING
    return low == (high & 0xE00000FFFFFFFFFF)? low : _mtbdd_makenode(var, low, high);
#else
...
MTBDD mtbdd_cube_nogc(MTBDD variables, uint8_t *cube, MTBDD terminal);
