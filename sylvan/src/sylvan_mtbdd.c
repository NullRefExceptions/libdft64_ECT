...
sylvan_init_mtbdd()
{
    ...
    /* 
    这部分工作包括
    1.TOGETHER(mtbdd_refs_init_task);
    2.sylvan_gc_add_mark(TASK(mtbdd_refs_mark));
    其中1可以延迟到spawn woker的时候执行，2可以在非lace环境下执行
    */
    sylvan_gc_add_mark(TASK(mtbdd_refs_mark));
}
...
MTBDD
_mtbdd_makenode_nogc(uint32_t var, MTBDD low, MTBDD high)
{
    ...
    if (index == 0) {
        fprintf(stderr, "BDD Unique table full! Try again with more memory or use slow mode. Abort...\n");
        exit(1);
    }
...
_mtbdd_makenode(uint32_t var, MTBDD low, MTBDD high)
{
    ...
    if (index == 0) {
        ...
        mtbdd_refs_push(high & 0xE00000FFFFFFFFFF);
#else
...
MTBDD
mtbdd_cube_nogc(MTBDD variables, uint8_t *cube, MTBDD terminal)
{
    ...
    switch (*cube) {
    case 0:
        result = mtbdd_cube_nogc(node_gethigh(variables, n), cube+1, terminal);
#if MTBDD_NODE_COUNTING
        pre_count = MTBDD_GETNODE(result)->a& 0x1FFFFF0000000000;
        pre_count += 0x10000000000;
        result = mtbdd_makenode_nogc(mtbddnode_getvariable(n), result, mtbdd_false|pre_count);
#else
        result = mtbdd_makenode_nogc(mtbddnode_getvariable(n), result, mtbdd_false);
#endif
        return result;
    case 1:
        result = mtbdd_cube_nogc(node_gethigh(variables, n), cube+1, terminal);
        ...
        pre_count = MTBDD_GETNODE(result)->a& 0x1FFFFF0000000000;
        pre_count += 0x10000000000;
        result = mtbdd_makenode_nogc(mtbddnode_getvariable(n), mtbdd_false, result|pre_count);
#else
        result = mtbdd_makenode_nogc(mtbddnode_getvariable(n), mtbdd_false, result);
#endif
        return result;
    case 2:
        return mtbdd_cube_nogc(node_gethigh(variables, n), cube+1, terminal);
    case 3:
    {
        MTBDD variables2 = node_gethigh(variables, n);
        mtbddnode_t n2 = MTBDD_GETNODE(variables2);
        uint32_t var2 = mtbddnode_getvariable(n2);
        result = mtbdd_cube_nogc(node_gethigh(variables2, n2), cube+2, terminal);
        BDD low = mtbdd_makenode_nogc(var2, result, mtbdd_false);
        BDD high = mtbdd_makenode_nogc(var2, mtbdd_false, result);
        result = mtbdd_makenode_nogc(mtbddnode_getvariable(n), low, high);
        return result;
    }
    default:
        return mtbdd_false; // ?
...
mtbdd_cube(MTBDD variables, uint8_t *cube, MTBDD terminal)
{
    ...
    case 0:
        ...
        pre_count = MTBDD_GETNODE(result)->a& 0x1FFFFF0000000000;
        pre_count += 0x10000000000;
        result = mtbdd_makenode(mtbddnode_getvariable(n), result, mtbdd_false|pre_count);
#else
    ...
    case 1:
        ...
        pre_count = MTBDD_GETNODE(result)->a& 0x1FFFFF0000000000;
        pre_count += 0x10000000000;
        result = mtbdd_makenode(mtbddnode_getvariable(n), mtbdd_false, result|pre_count);
#else
...
#if MTBDD_NODE_COUNTING
fprintf(out, "%" PRIu64 " [label=\"var:%" PRIu32 ",bdd:%lu,count:%lu\"];\n",
                MTBDD_STRIPMARK(mtbdd), mtbddnode_getvariable(n),MTBDD_STRIPMARK(mtbdd),(n->a&0x1FFFFF0000000000)>>40);
#else
